...
class PrefectETLPipelineConfig:
    ...
    def _load_config(self) -> None:
        try:
            self._internal_logger.debug(
                "prefect_configuration_loading path=%s", self._config_path
            )
            ...
            self._internal_logger.error(
                "prefect_configuration_load_failed error=%s", exc, exc_info=True
            )
    ...
    def get_postgresql_schema_dw(self) -> str:
        return self.get_postgres_config().get("dimensions", "homecredit_lending")
